...
type FileNode = { id: string; name: string; path: string; type: 'dir' | 'file'; size: string; perms: string; modified: string; children?: FileNode[] };
...
function normalizeShell(shell: WebShell): WebShell { const type = String(shell.type || '').toLowerCase(); return { ...shell, ID: Number(shell.ID), type: (['php', 'java', 'c#', 'asp'].includes(type) ? type : 'c#') as ShellType }; }
const displayShellType = (type: string) => type;
...
function joinPath(base: string, name: string) { const clean = base || '/'; return clean.endsWith('/') ? `${clean}${name}` : `${clean}/${name}`; }
function parentPath(path: string) { const clean = path.replace(/\/+$/, ''); const idx = clean.lastIndexOf('/'); return idx <= 0 ? '/' : clean.slice(0, idx); }
...
function ShellWorkspace({ shell, module, refreshList }: { shell: WebShell; module: ModuleKey; refreshList: () => Promise<number> }) {
  ...
  useEffect(() => { setSystemInfo(''); setCodeOutput(''); setCode('echo "CloudDrop";'); loadSystemInfo(); }, [shell.ID]);
...
function CodePanel({ code, setCode, output, busy, onRun }: { code: string; setCode: (value: string) => void; output: string; busy: boolean; onRun: () => void }) { return <section className="panel ide-panel"><div className="panel-head"><div><span className="eyebrow">Code Runner</span><h2>代码执行</h2></div><button className="button primary" type="button" onClick={onRun} disabled={busy}><Icon name="code" />{busy ? '执行中' : '执行代码'}</button></div><div className="code-runner"><label><span>Payload</span><textarea className="code-editor" value={code} onChange={(event) => setCode(event.target.value)} rows={14} /></label><div><span className="field-title">返回结果</span><SyntaxHighlighter language="php" style={oneLight} customStyle={syntaxStyle}>{output || '执行结果会显示在这里。'}</SyntaxHighlighter></div></div></section>; }

function FileManager({ shell }: { shell: WebShell }) {
  const [path, setPath] = useState(shell.type === 'php' ? '/app/' : '/tmp/'); const [nodes, setNodes] = useState<FileNode[]>([]); const [selected, setSelected] = useState<FileNode | null>(null); const [content, setContent] = useState(''); const [busy, setBusy] = useState(''); const [error, setError] = useState(''); const [treeHeight, setTreeHeight] = useState(Math.max(520, window.innerHeight - 180));
  const loadPath = async (nextPath = path) => { const normalizedPath = nextPath === '/' || nextPath.endsWith('/') ? nextPath : `${nextPath}/`; setBusy('list'); setError(''); try { const res = await postForm(`/webshells/FileList/${shell.ID}`, { path: normalizedPath }); const parsed = parseFileList(String(res.data.info ?? ''), normalizedPath); setNodes(parsed); setPath(normalizedPath); setSelected(null); setContent(''); } catch (err) { setError(readError(err)); } finally { setBusy(''); } };
  const openNode = async (node: FileNode) => { setSelected(node); if (node.type === 'dir') { await loadPath(node.path.endsWith('/') ? node.path : `${node.path}/`); return; } setBusy('show'); setError(''); try { const res = await postForm(`/webshells/FileShow/${shell.ID}`, { path: node.path }); setContent(String(res.data.info ?? '')); } catch (err) { setContent(readError(err)); } finally { setBusy(''); } };
  const runArchive = async (mode: 'zip' | 'unzip') => { const defaultSrc = selected?.path || path; const srcPath = window.prompt(mode === 'zip' ? '输入要压缩的源路径' : '输入要解压的压缩包路径', defaultSrc); if (!srcPath) return; const defaultTo = mode === 'zip' ? `${srcPath.replace(/\/$/, '')}.zip` : path; const toPath = window.prompt(mode === 'zip' ? '输入压缩包输出路径' : '输入解压目标目录', defaultTo); if (!toPath) return; setBusy(mode); setError(''); try { const endpoint = mode === 'zip' ? 'FileZip' : 'FileUnZip'; const res = await postForm(`/webshells/${endpoint}/${shell.ID}`, { srcPath, toPath }); setContent(String(res.data.info ?? '执行完成')); await loadPath(path); } catch (err) { setError(readError(err)); } finally { setBusy(''); } };
  ...
  useEffect(() => { loadPath(path); }, [shell.ID]);
...
function MiniMetric({ label, value, tone = 'blue' }: { label: string; value: number; tone?: string }) { return <article className={`mini-metric ${tone}`}><span>{label}</span><strong>{value}</strong></article>; }
...
function parseFileList(raw: string, currentPath: string): FileNode[] { const lines = raw.split(/\r?\n/).filter(Boolean).slice(2); return lines.map((line) => { const [rawName, size = '', perms = '', modified = ''] = line.split('\t'); const type: FileNode['type'] = rawName.startsWith('dic:') ? 'dir' : 'file'; const name = rawName.replace(/^dic:/, ''); const path = joinPath(currentPath, name); return { id: path, name, path, type, size, perms, modified, children: type === 'dir' ? [] : undefined }; }).sort((a, b) => a.type === b.type ? a.name.localeCompare(b.name) : a.type === 'dir' ? -1 : 1); }
function parseMaybeJson(raw: string): unknown { try { return JSON.parse(raw); } catch { return raw; } }
function guessLanguage(name?: string) { if (!name) return 'text'; if (name.endsWith('.php')) return 'php'; if (name.endsWith('.jsp') || name.endsWith('.java')) return 'java'; if (name.endsWith('.js')) return 'javascript'; if (name.endsWith('.json')) return 'json'; if (name.endsWith('.html')) return 'html'; if (name.endsWith('.css')) return 'css'; return 'text'; }
