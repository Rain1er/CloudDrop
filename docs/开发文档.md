...
- 计划支持dotnet的各种trick，Java内存马。后面联动nuclei和bp进行自动getshell，这里注意net的内存马需要发送心跳包维持存活，因为其有gc机制。
- 计划支持suo5正向代理  
- 静态页面欺骗，可以参考下 https://github.com/xiaogang000/XG_NTAI

## 2025-6-29
- 完成了class的编译
- 完善了session管理，可以通过心跳包来维持session的存活(不需要持续心跳，只需要在测试存活的时候发送一次head即可)。暂未测试是否有bug


## 2025-7-18

web 文件打包https://mp.weixin.qq.com/s/YaRQH3uU_7Y9u4EHSNmX8w  

命令执行增强 https://mp.weixin.qq.com/s/MY8auZG64h-xyf3Qg1aYIw
